  ...
  return (
    <section className="battle-arena rounded-[2rem] border border-white/80 p-4 shadow-xl sm:p-6">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          <div className="text-[10px] font-bold tracking-[0.25em] text-rose-400">DUEL ARENA</div>
        ...
        <div className="rounded-2xl bg-white/70 px-3 py-2 text-right text-xs text-slate-500 backdrop-blur">
          <div>내 승리 확률</div>
          <b className="text-lg text-rose-500">{Math.round(chance * 100)}%</b>
        </div>
      </div>
      ...
      <div className="relative grid min-h-[270px] grid-cols-[1fr_auto_1fr] items-end gap-2 sm:gap-6">
        ...
        <div className={`relative flex flex-col items-center justify-end transition ${phase === 'win' ? 'battle-hero-lose' : phase === 'lose' ? 'battle-hero-win' : ''}`}>
          <div className="mb-1 rounded-full bg-rose-500/10 px-3 py-1 text-[10px] font-bold text-rose-600">RIVAL HERO</div>
          <HeroPreview hero={opponent} size={172} animated action={opponentAction} />
